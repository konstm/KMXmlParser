...
NSArray* PerformXPathQueryInNameSpace(xmlDocPtr doc, NSString *query, NSString *ns_prefix, NSString *ns_uri) {
    ...
    if (ns_prefix != nil && ns_uri != nil)
        xmlXPathRegisterNs(xpathCtx, BAD_CAST[ns_prefix cStringUsingEncoding:NSUTF8StringEncoding], BAD_CAST[ns_uri cStringUsingEncoding:NSUTF8StringEncoding]);
...
{
    return PerformXPathQueryInNameSpace(doc, query, nil, nil);
...
NSArray* PerformXMLXPathQueryInNamespace(NSData *document, NSString *query, NSString *ns_prefix, NSString *ns_uri) {
	...
	NSArray *result = PerformXPathQueryInNameSpace(doc, query, ns_prefix, ns_uri);
    xmlFreeDoc(doc); 
